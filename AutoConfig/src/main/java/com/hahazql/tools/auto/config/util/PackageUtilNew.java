package com.hahazql.tools.auto.config.util;
...
/**
 * Created by zql on 2015/10/21.
 */
public class PackageUtilNew {
    private static HashMap<URL, ClassLoader> _url_ClassLoader = new HashMap<URL, ClassLoader>();
    ...
    public static ClassLoader getClassLoader(String jarPath) throws IOException, NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        ...
        if (_url_ClassLoader.containsKey(jarUrl))
            return _url_ClassLoader.get(jarUrl);
        ...
        synchronized (lockCreateLoader) {
            ...
            _url_ClassLoader.put(jarUrl, cl);
        }
        return cl;
    }
     ...
     */
    public static List<Class> getClassFromJar(String jarPath, ClassLoader classLoader) throws IOException, NoSuchMethodException, IllegalAccessException, InvocationTargetException {
            ...
            while (true) {
                ...
                if ((crunchifyJar.getName().endsWith(".class"))) {
                    ...
                    try {
                        ...
                        boolean isAnnotationPresent = clazz.isAnnotationPresent(ClazzConfig.class);
                    } catch (Exception e) {
                        /** 无法loader Class进来**/
                        e.printStackTrace();
                    }
    ...
    public static List<String> getClassNameByFile(String filePath, List<String> className, boolean childPackage, String packageName) {
        ...
        for (File childFile : childFiles) {
            if (childFile.isDirectory()) {
                if (childPackage) {
                    myClassName.addAll(getClassNameByFile(childFile.getPath(), myClassName, true, packageName));
                }
            } else {
        ...
        for (File childFile : childFiles) {
            if (childFile.isDirectory()) {
                ...
                if (childFilePath.endsWith(".class")) {
                    ...
                    } else if (childFilePath.contains("\\classes")) {
                        childFilePath = childFilePath.substring(childFilePath.indexOf("\\classes") + 9, childFilePath.lastIndexOf("."));
                        childFilePath = childFilePath.replace("\\", ".");
                    } else if (childFilePath.contains("/bin")) {
