...
public class ClassUtil {
     ...
     * <p/>
     * <p>
     * 相当于 <code>com.hahazql.util.clazz.getName()</code> ，但不同的是，该方法用更直观的方式显示数组类型。 例如：
     * <p/>
     * <pre>
     *  int[].class.getName() = "[I" ClassUtil.getClassName(int[].class) = "int[]"
     *
     ...
     * <p/>
     * <p>
     * <code>className</code> 必须是从 <code>com.hahazql.util.clazz.getName()</code>
     ...
     * <li>
     * <code>java.com.hahazql.util.io.Serializable</code></li>
     * </ol>
     * </p>
     * <p/>
     * <p>
     ...
     * <code>classes</code> 的每个元素 <code>com.hahazql.util.clazz</code>， 按照如下规则：
     * <p/>
     * <ol>
     * <li>
     * 如果目标类 <code>com.hahazql.util.clazz</code> 为 <code>null</code> ，总是返回 <code>false</code>。</li>
     * <li>
     * 如果参数类型 <code>fromClass</code> 为 <code>null</code> ，并且目标类型
     * <code>com.hahazql.util.clazz</code> 为非原子类型，则返回 <code>true</code>。 因为 <code>null</code>
     * 可以被赋给任何引用类型。</li>
     * <li>
     * 调用 <code>Class.isAssignableFrom</code> 方法来确定目标类 <code>com.hahazql.util.clazz</code> 是否和参数类
     * <code>fromClass</code> 相同或是其父类、接口，如果是，则返回 <code>true</code>。</li>
     * <li>
     * 如果目标类型 <code>com.hahazql.util.clazz</code> 为原子类型，那么根据 <a
     ...
     * <code>com.hahazql.util.clazz</code> 为 <code>long</code> ，那么参数类型可以是 <code>byte</code>、
    ...
    /**
     * 检查指定类型 <code>fromClass</code> 的对象是否可以赋值给另一种类型 <code>com.hahazql.util.clazz</code>。
     * <p/>
     * <p>
    ...
    protected static TypeInfo getTypeInfo(Class type) {
        if (type == null) {
            throw new IllegalArgumentException("Parameter com.hahazql.util.clazz should not be null");
        }
    ...
    public static void setFieldValue(Object o, String fieldName, Object value) throws InvocationTargetException, IllegalAccessException {
        StringBuffer fnName = new StringBuffer(fieldName.substring(0, 1).toUpperCase());
        fnName.append(fieldName.substring(1));
        String methodName = String.format("set%s", fnName.toString());
        Method setMethod = ClassUtil.getMethod(o.getClass(), methodName);
        setMethod.invoke(o, value);
    }

    public static <T> T getFieldValue(Object o, String fieldName, Class<T> fieldClazz) throws InvocationTargetException, IllegalAccessException {
        StringBuffer fnName = new StringBuffer(fieldName.substring(0, 1).toUpperCase());
        if (fieldName.length() > 1)
            fnName.append(fieldName.substring(1));
        String methodName = String.format("get%s", fnName.toString());
        Method setMethod = ClassUtil.getMethod(o.getClass(), methodName);
        return (T) setMethod.invoke(o);
    }
