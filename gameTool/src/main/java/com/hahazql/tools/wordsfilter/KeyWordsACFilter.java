package com.hahazql.tools.wordsfilter;
...
public class KeyWordsACFilter implements IKeyWordsFilter {
    ...
    public static KeyWordsACFilter getInstance(char subtitute, String... keyWords) {
        KeyWordsACFilter filter = new KeyWordsACFilter(subtitute, null);
        filter.initialize(keyWords);
        return filter;
    }

    public static KeyWordsACFilter getInstance(char subtitute, String[] keyWords, char... ignore) {
        KeyWordsACFilter filter = new KeyWordsACFilter(subtitute, ignore);
        filter.initialize(keyWords);
        return filter;
    }
     ...
     */
    public KeyWordsACFilter(char substitute, char... ignore) {
        ...
        if (ignore != null) {
            ...
            this.ignoreChars = new char[1];
            this.ignoreChars[0] = '\0';
        }
    ...
    private final void buildFailNode() {
        ...
        Iterator<DFANode> _it = dfaEntrance.dfaTransition.values().iterator();
        while (_it.hasNext()) {
            DFANode node = _it.next();
            node.level = 1;
            queues.add(node);
